...
RSpec.describe RoomBookingsHelper, type: :helper do
    ...
    let!(:time_slot4) { create(:time_slot,day: 'MWF', start_time: '8:00', end_time: '9:00') }
    let!(:time_slot5) { create(:time_slot,day: 'MW', start_time: '8:00', end_time: '9:00') }
    ...
    let!(:preference12) { create(:instructor_preference, instructor: instructor1, course: course2, preference_level: '3') }
    let!(:preference21) { create(:instructor_preference, instructor: instructor2, course: course, preference_level: '3') }
    let!(:preference22) { create(:instructor_preference, instructor: instructor2, course: course2, preference_level: '3') }
    let!(:preference31) { create(:instructor_preference, instructor: instructor3, course: course, preference_level: '4') }
    let!(:preference32) { create(:instructor_preference, instructor: instructor3, course: course2, preference_level: '3') }
    ...
    let!(:room_booking31) { create(:room_booking, room: room1, time_slot: time_slot3, section: section3,  is_available: true) }
    let!(:room_booking41) { create(:room_booking, room: room1, time_slot: time_slot4, section: section2,  is_available: true) }
    let!(:room_booking51) { create(:room_booking, room: room1, time_slot: time_slot5, section: section5,  is_available: true) }
    let!(:room_booking32) { create(:room_booking, room: room2, time_slot: time_slot3, section: section6,  is_available: true) }
    let!(:room_booking42) { create(:room_booking, room: room2, time_slot: time_slot4, section: section4,  is_available: true) }
    let!(:room_booking52) { create(:room_booking, room: room2, time_slot: time_slot5, section:,  is_available: true) }
    # before do
    ...
    describe 'eligible_instructors' do
      ...
      it "sorts teachers by prefrence, then by bandidth" do 
        ans = eligible_instructors(schedule, room_booking1)
