...
RSpec.describe RoomBookingsHelper, type: :helper do
  ...
  let!(:instructor2) do
    create(:instructor, schedule:, first_name: 'Robert', last_name: 'Across', before_9: false, after_3: true, max_course_load: 1)
  end
  let!(:instructor3) { create(:instructor, schedule:, first_name: 'Robby', last_name: 'Loss', before_9: true, after_3: true, max_course_load: 2) }
  let!(:preference11) { create(:instructor_preference, instructor: instructor1, course:, preference_level: '5') }
  let!(:preference12) { create(:instructor_preference, instructor: instructor1, course: course4, preference_level: '3') }
  let!(:preference21) { create(:instructor_preference, instructor: instructor2, course:, preference_level: '3') }
  let!(:preference22) { create(:instructor_preference, instructor: instructor2, course: course4, preference_level: '3') }
  let!(:preference31) { create(:instructor_preference, instructor: instructor3, course:, preference_level: '4') }
  ...
  let!(:room_booking31) { create(:room_booking, room: room1, time_slot: time_slot3, course:, is_available: true) }
  ...
  let!(:room_booking52) { create(:room_booking, room: room2, time_slot: time_slot1, course:, is_available: true) }
  # before do
