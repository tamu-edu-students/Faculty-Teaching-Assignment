...
When('I visit the instructor page for id {string}') do |schedule_id|
  ...
  Given('I am on the details page for {string}') do |schedule_name|
    visit schedule_path(id: @schedule.id) # Adjust this based on your routes
  end
  

  When('I attach a valid CSV file with path {string}') do |file_path|
    attach_file('instructor_file', Rails.root.join(file_path))
  end
